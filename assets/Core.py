...
class Asset(object):
    ...
    def mandatory_check(self, data, only_check_sn=False):
        """检查数据库中提交的资产是否存在, 如果存在返回id, 否则保存的零时资产表等待审批"""
        ...
        try:
            ...
            self.waiting_approval = True  # 设置这条次产为待批准
            return False
    ...
    def get_asset_id_by_sn(self):
        ...
        if data:
            try:
                ...
                if self.mandatory_check(data, only_check_sn=True):  # 如果资产已经在数据库中存在，则向客户端返回assist id
    ...
    def save_new_asset_to_approval_zone(self):
        """将资产信息保存到临时审批表等待管理员审批"""
        asset_sn = self.clean_data.get('sn')
        save_data = {"sn": asset_sn,
        ...
        asset_already_in_approval_zone = models.NewAssetApprovalZone.objects.get_or_create(**save_data)
        return True
    ...
    def __verify_field(self, data_set, field_key, data_type, required=True):
        '''
        验证字段
        :param data_set:采集到的数据
        :param field_key: 要验证的字段
        ...
        try:
            ...
            if hasattr(component_obj, 'select_related'):  # this component is reverse m2m relation with Asset model
                ...
                for obj in objects_from_db:
                    key_field_data = getattr(obj, identify_field)  # 获取表的唯一标识的字段
