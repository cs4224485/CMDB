...
def build_table_row(row_obj, table_obj, onclick_column=None, target_link=None):
    ...
    for index, column_name in enumerate(table_obj.list_display):
        try:
            ...
            column_data = row_obj._meta.get_field(column_name).value_to_string(row_obj)
            # column_data = row_obj._meta.get_field(column_name)._get_val_from_obj(row_obj) # Django 1.0
        except Exception as e:
                ...
                for index, column_name in enumerate(table_obj.dynamic_list_display):
                    if hasattr(dy_fk_obj, column_name):
                        if column_name in table_obj.dynamic_choice_fields:
                            ...
                            # column_data = dy_fk_obj._meta.get_field(column_name)._get_val_from_obj(dy_fk_obj) Django 1.0
                            column_data = dy_fk_obj._meta.get_field(column_name).value_to_string(dy_fk_obj)
